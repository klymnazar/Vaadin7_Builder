...
//
////		flatGrid.addColumn("flatContractDate", String.class);
//
...
//					, flatFromList.getFlatCost(), flatFromList.getFlatCost_m(),
...
//					flatFromList.getFlatContractDate(), flatFromList.getFlatSellerName(), flatFromList.getFlatNotes()
					);
		}
...
//		buttonHorizontalLayout.addComponent(cancelButton());
		...
		if (buyerInfoCheckBox.isEmpty()) {
			...
			flatGrid.removeColumn("flatCost");
		}
		else {
			flatGrid.addColumn("flatCost", Integer.class);
//			flatService.updateBuyerInfoFlatByFlatId(flatId, flat);
		...
		CheckBox expensesInfoCheckBox = new CheckBox("Expenses Info");
		checkBoxHorizontalLayout.addComponent(expensesInfoCheckBox);
		
		Button showCheckBoxButton = new Button("Show");
		checkBoxHorizontalLayout.addComponent(showCheckBoxButton);
...
//					.getItemProperty("idFlatTable").getValue().toString();
			...
			addSpendButton.addClickListener(e1 -> {
				spendGrid.addRow(Integer.parseInt(idFlatTableFromSelectedRow), Integer.parseInt(spendSumTextField.getValue()), spendCurrencyComboBox.getValue(), spendCategoryComboBox.getValue(), spendValueComboBox.getValue() + " " + spendValueTextArea.getValue());
