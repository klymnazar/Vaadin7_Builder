...
//		flatGrid = flatGridFlatInfoView(flatService.getFlatsFromFlatTableAndFlatBuyerDB());
	...
	public Grid flatGridFlatInfoView(List<Flat> flatList) throws SQLException {
		...
		while (itr.hasNext()) {
			...
			flatFromBuyerTable = flatService.getFlatByFlatIdFromFlatBuyerTable(flatFromList.getIdFlatTable());
			...
			flatGrid.addRow(number, flatFromList.getIdFlatTable(), flatFromList.getBuildingCorps(),
					...
					flatFromList.getFlatArea(), flatFromList.getFlatSet(), flatFromBuyerTable.getFlatContractDate() // flatFromList.getFlatContractDate()
			);
			number++;
...
//////////////////////////
	...
	public Button saleFlatButton() throws SQLException {
		...
		saleFlatButton.addClickListener(e -> {
			...
			HorizontalLayout saleFlatWindowInfoHorizontalLayout = new HorizontalLayout();
			saleFlatWindowInfoHorizontalLayout.setSizeFull();
			saleFlatWindowInfoHorizontalLayout.setSpacing(true);
			...
			saleFlatWindowFormLayout.addComponent(notesTextArea);
			...
			saleFlatWindowInfoHorizontalLayout.addComponent(saleFlatInfoGridPanel);
